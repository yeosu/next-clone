...
export default function TweetModal() {
  ...
  const mutation = useMutation({
    ...
    async onSuccess(response: { json: () => any; }, variable: any) {
      ...
      queryKeys.forEach((queryKey: any[]) => {
        if (queryKey[0] === 'posts') {
          ...
          if (value && 'pages' in value) {
            ...
            const obj = value.pages.flat().find((v: { postId: number | undefined; }) => v.postId === parent?.postId);
            if (obj) { // 존재는 하는지
              const pageIndex = value.pages.findIndex((page: string | any[]) => page.includes(obj));
              const index = value.pages[pageIndex].findIndex((v: { postId: number | undefined; }) => v.postId === parent?.postId);
