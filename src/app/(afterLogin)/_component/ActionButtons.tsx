...
export default function ActionButtons({ white, post }: Props) {
  ...
  const heart = useMutation({
    ...
    onMutate() {
      ...
      queryKeys.forEach((queryKey: any[]) => {
        if (queryKey[0] === 'posts') {
          console.log(queryKey[0]);
          const value: Post | InfiniteData<Post[]> | undefined = queryClient.getQueryData(queryKey);
          if (value && 'pages' in value) {
            console.log('array', value);
            const obj = value.pages.flat().find((v: { postId: number; }) => v.postId === postId);
            if (obj) { // 존재는 하는지
              const pageIndex = value.pages.findIndex((page: string | any[]) => page.includes(obj));
              const index = value.pages[pageIndex].findIndex((v: { postId: number; }) => v.postId === postId);
        ...
        if (queryKey[0] === 'posts') {
          ...
          if (value && 'pages' in value) {
            ...
            if (obj) { // 존재는 하는지
              ...
              shallow.pages[pageIndex][index] = {
                ...shallow.pages[pageIndex][index],
                Hearts: shallow.pages[pageIndex][index].Hearts.filter((v: { userId: string | null | undefined; }) => v.userId !== session?.user?.email),
                _count: {
                  ...shallow.pages[pageIndex][index]._count,
                  Hearts: shallow.pages[pageIndex][index]._count.Hearts - 1,
                }
            ...
            if (value.postId === postId) {
              const shallow = {
                ...value,
                Hearts: value.Hearts.filter((v: { userId: string | null | undefined; }) => v.userId !== session?.user?.email),
                _count: {
                  ...value._count,
                  Hearts: value._count.Hearts - 1,
                }
  ...
  const repost = useMutation({
    ...
    async onSuccess(response: { json: () => any; }) {
      ...
      queryKeys.forEach((queryKey: string[]) => {
        if (queryKey[0] === 'posts') {
          ...
          if (value && 'pages' in value) {
            console.log('array', value);
            const obj = value.pages.flat().find((v: { postId: number; }) => v.postId === postId);
            const repost = value.pages.flat().find((v: { Original: { postId: number; }; User: { id: string | null | undefined; }; }) => v.Original?.postId === postId && v.User.id === session?.user?.email);
            if (obj) { // 존재는 하는지
              const pageIndex = value.pages.findIndex((page: string | any[]) => page.includes(obj));
              const index = value.pages[pageIndex].findIndex((v: { postId: number; }) => v.postId === postId);
              console.log('found index', index);
              ...
              shallow.pages[pageIndex][index] = {
                ...shallow.pages[pageIndex][index],
                Reposts: shallow.pages[pageIndex][index].Reposts.filter((v: { userId: string | null | undefined; }) => v.userId !== session?.user?.email),
                _count: {
                  ...shallow.pages[pageIndex][index]._count,
                  Reposts: shallow.pages[pageIndex][index]._count.Reposts - 1,
                }
              ...
              // 재게시 삭제
              shallow.pages = shallow.pages.map((page: any[]) => {
            ...
            if (value.postId === postId) {
              const shallow = {
                ...value,
                Reposts: value.Reposts.filter((v: { userId: string | null | undefined; }) => v.userId !== session?.user?.email),
                _count: {
                  ...value._count,
                  Reposts: value._count.Reposts - 1,
                }
